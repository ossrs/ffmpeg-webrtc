...
int32_t g_yang_mt_connectServer(void* peer,int32_t pmediaServer){
	...
	if(conn==NULL) return ERROR_RTC_PEERCONNECTION;
...
int32_t g_yang_mt_onVideo(void* peer,YangFrame* videoFrame){
...
int32_t g_yang_mt_onAudio(void* peer,YangFrame *audioFrame){
...
int32_t g_yang_mt_onMessage(void* peer,YangFrame *msgFrame){
	if(peer==NULL||msgFrame==NULL) return  ERROR_RTC_PEERCONNECTION;
	YangMetaSession *session = (YangMetaSession*) peer;
...
void yang_create_metaConnection(YangMetaConnection* metaconn){
	...
	metaconn->on_audio=g_yang_mt_onAudio;
	metaconn->on_video=g_yang_mt_onVideo;
