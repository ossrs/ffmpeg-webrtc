...
/* The default size of the rtp history buffer.*/
#define HISTORY_SIZE_DEFAULT 512

#define SSRC_GENERATOR_MAX_ATTEMPTS 100
...
static int parse_profile_level(AVFormatContext *s, AVCodecParameters *par)
{
    ...
    if (!par->extradata || par->extradata_size < 8) {
...
static uint32_t generate_unique_ssrc(WHIPContext *whip)
{
    ...
    int max_attempts = SSRC_GENERATOR_MAX_ATTEMPTS;
    int is_unique = 1, i;

    do {
        // Truncate to 31 bits to avoid overflow when passed into rtpenc as signed int
        candidate = av_lfg_get(&whip->rnd) & 0x7fffffff;
    ...
    av_log(whip, AV_LOG_ERROR, "Failed to generate unique SSRC after %d attempts\n", SSRC_GENERATOR_MAX_ATTEMPTS);
    return candidate;
}
...
static int generate_sdp_offer(AVFormatContext *s)
{
    ...
    whip->video_rtx_ssrc = generate_unique_ssrc(whip);
    whip->rtx_seq = 0;
    whip->hist_head = 0;
...
static int rtp_history_store(WHIPContext *whip, const uint8_t *buf, int size)
{
    ...
    uint32_t pos = ((uint32_t)seq - (uint32_t)whip->video_first_seq) % whip->history_size;
    RtpHistoryItem *it = &whip->history[pos];
    /* free older entry */
    av_free(it->buf);
    it->buf = av_memdup(buf, size);
    if (!it->buf)
        return AVERROR(ENOMEM);
    it->size = size;
    it->seq = seq;
...
{
    uint32_t pos = ((uint32_t)seq - (uint32_t)whip->video_first_seq) % whip->history_size;
    const RtpHistoryItem *it = &whip->history[pos];
    if (it->seq == seq)
        return it;
    return NULL;
}
...
static void handle_rtx_packet(AVFormatContext *s, int seq)
{
    ...
    uint8_t *orig_buf, out_buf[MAX_UDP_BUFFER_SIZE];
    int orig_size;
    ...
    if (!it) {
        av_log(whip, AV_LOG_WARNING,
            "RTX: Packet seq=%d not found, latest seq=%d, latest rtx seq=%d\n",
            seq, latest_seq, whip->rtx_seq);
        return;
    }
    av_log(whip, AV_LOG_VERBOSE,
        "RTX: Resending packet, size=%d, seq=%d, lateset stored packet seq=%d, latest rtx seq=%d\n",
        it->size, seq, latest_seq, whip->rtx_seq);

    orig_buf = it->buf;
    orig_size = it->size;

    /* new buffer: header + 2 + payload */
    if (orig_size + 2 > sizeof(whip->buf)) {
        av_log(whip, AV_LOG_WARNING, "RTX packet is too large, size=%d\n", orig_size);
        goto end;
    }

    memcpy(whip->buf, orig_buf, orig_size);
    ...
    /* Encrypt by SRTP and send out. */
    cipher_size = ff_srtp_encrypt(&whip->srtp_video_rtx_send, whip->buf, new_size, out_buf, sizeof(out_buf));
    if (cipher_size <= 0) {
        av_log(whip, AV_LOG_WARNING, "Failed to encrypt packet=%dB, cipher=%dB\n", new_size, cipher_size);
        goto end;
    }
    ret = ffurl_write(whip->udp, out_buf, cipher_size);
end:
    if (ret < 0)
        av_log(whip, AV_LOG_WARNING, "Failed to send RTX packet, try to next one\n");
}
...
static void process_nack_rtx(AVFormatContext *s, int size)
{
    ...
    WHIPContext *whip = s->priv_data;
    uint8_t *buf = NULL;
    int rtcp_len, srtcp_len, header_len = 12;
    ...
    if (rtcp_len <= header_len) {
        av_log(whip, AV_LOG_WARNING, "NACK packet(SRTCP) is broken, rtcp_len: %d\n", rtcp_len);
        goto error;
    }
    /* SRTCP index(4 bytes) + HMAC (SRTP_AES128_CM_SHA1_80 10bytes) */
    srtcp_len = rtcp_len + 4 + 10;
    if (srtcp_len != size) {
        av_log(whip, AV_LOG_WARNING, "NACK packet(SRTCP) size not match, srtcp_len: %d, size: %d\n", srtcp_len, size);
        goto error;
    }
    ...
    if (ret < 0) {
        av_log(whip, AV_LOG_WARNING, "NACK packet(SRTCP) decrypt failed: %d\n", ret);
        goto error;
    }
    ...
    goto end;
error:
    av_log(whip, AV_LOG_WARNING, "Skip to handle NACK and RTX\n");
end:
    if (buf)
        av_freep(&buf);
    return;
...
static av_cold void whip_deinit(AVFormatContext *s)
{
    ...
    for (i = 0; i < whip->history_size; i++) {
        av_freep(&whip->history[i].buf);
    }
