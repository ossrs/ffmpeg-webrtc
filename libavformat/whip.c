...
static int generate_sdp_offer(AVFormatContext *s)
{
    ...
    if (whip->rtc.audio_par) {
        bundle[bundle_index++] = '0';
        bundle[bundle_index++] = ' ';
    }
    if (whip->rtc.video_par) {
    ...
    if (whip->rtc.audio_par) {
        if (whip->rtc.audio_par->codec_id == AV_CODEC_ID_OPUS)
            acodec_name = "opus";
        ...
        av_bprintf(&bp, ""
            ...
            whip->audio_payload_type,
            acodec_name,
            whip->rtc.audio_par->sample_rate,
            whip->rtc.audio_par->ch_layout.nb_channels,
            whip->audio_ssrc,
            whip->audio_ssrc);
    }

    if (whip->rtc.video_par) {
        level = whip->rtc.video_par->level;
        if (whip->rtc.video_par->codec_id == AV_CODEC_ID_H264) {
            vcodec_name = "H264";
            profile_iop |= whip->rtc.video_par->profile & AV_PROFILE_H264_CONSTRAINED ? 1 << 6 : 0;
            profile_iop |= whip->rtc.video_par->profile & AV_PROFILE_H264_INTRA ? 1 << 4 : 0;
            profile_idc = whip->rtc.video_par->profile & 0x00ff;
        }
