...
static void* dtls_context_new(AVClass *av_class, void *opaque, int pkt_size, dtls_fn_on_state on_state, dtls_fn_on_write on_write, const char* cert_file, const char* key_file);
static av_cold int dtls_context_init(AVFormatContext *s, void *ctx);
static int dtls_context_start(void *ctx);
static int dtls_context_write(void *ctx, char* buf, int size);
static av_cold void dtls_context_deinit(void *ctx);

static int dtls_can_handle_packet(uint8_t *b, int size);
static char* dtls_get_fingerprint(void *ctx);
static uint8_t* dtls_get_srtp_client_key(void *ctx);
static uint8_t* dtls_get_srtp_server_key(void *ctx);
static uint8_t* dtls_get_srtp_client_salt(void *ctx);
static uint8_t* dtls_get_srtp_server_salt(void *ctx);
 ...
 */
static int dtls_context_on_state(void *pctx, void *opaque, int state, const char* type, const char* desc)
{
    int ret = 0;
 ...
 */
static int dtls_context_on_write(void *ctx, void *opaque, char* data, int size)
{
    AVFormatContext *s = opaque;
    WHIPContext *whip = s->priv_data;
...
static av_cold int initialize(AVFormatContext *s)
{
    ...
    whip->dtls_ctx = dtls_context_new(whip->av_class, s, whip->pkt_size, dtls_context_on_state, dtls_context_on_write, whip->cert_file, whip->key_file);
...
static int generate_sdp_offer(AVFormatContext *s)
{
    ...
    if (whip->audio_par) {
        ...
        av_bprintf(&bp, ""
            ...
            whip->ice_pwd_local,
            dtls_get_fingerprint(whip->dtls_ctx),
            whip->audio_payload_type,
            acodec_name,
...
next_packet:
        ...
        if (dtls_can_handle_packet(whip->buf, ret) && whip->state >= WHIP_STATE_ICE_CONNECTED) {
...
static int setup_srtp(AVFormatContext *s)
{
     ...
     */
    char *client_key = dtls_get_srtp_client_key(whip->dtls_ctx);
    char *server_key = dtls_get_srtp_server_key(whip->dtls_ctx);
    char *client_salt = dtls_get_srtp_client_salt(whip->dtls_ctx);
    char *server_salt = dtls_get_srtp_server_salt(whip->dtls_ctx);
...
static int whip_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    ret = ffurl_read(whip->udp_uc, whip->buf, sizeof(whip->buf));
    if (ret > 0) {
        if (dtls_can_handle_packet(whip->buf, ret)) {
            if ((ret = dtls_context_write(whip->dtls_ctx, whip->buf, ret)) < 0) {
...
static char* dtls_get_fingerprint(void *pctx)
{
    DTLSContext *ctx = pctx;
    return ctx->dtls_fingerprint;
}

static uint8_t* dtls_get_srtp_client_key(void *pctx)
{
    DTLSContext *ctx = pctx;
    return ctx->dtls_srtp_materials;
}

static uint8_t* dtls_get_srtp_server_key(void *pctx)
{
    return dtls_get_srtp_client_key(pctx) + DTLS_SRTP_KEY_LEN;
}

static uint8_t* dtls_get_srtp_client_salt(void *pctx)
{
    return dtls_get_srtp_server_key(pctx) + DTLS_SRTP_KEY_LEN;
}

static uint8_t* dtls_get_srtp_server_salt(void *pctx)
{
 ...
 */
static av_cold int dtls_context_init(AVFormatContext *s, void *pctx)
{
    DTLSContext *ctx = pctx;
 ...
 */
static int dtls_context_start(void *pctx)
{
    DTLSContext *ctx = pctx;
 ...
 * @return 0 if OK, AVERROR_xxx on error
 */
static int dtls_context_write(void *pctx, char* buf, int size)
{
    DTLSContext *ctx = pctx;
 ...
 */
static av_cold void dtls_context_deinit(void *pctx)
{
    DTLSContext *ctx = pctx;
