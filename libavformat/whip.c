...
static av_cold int initialize(AVFormatContext *s)
{
    ...
    av_lfg_init(&whip->rtc.rnd, seed);

    /* 64 bit tie breaker for ICE-CONTROLLING (RFC 8445 16.1) */
    ret = av_random_bytes((uint8_t *)&whip->rtc.ice_tie_breaker, sizeof(whip->rtc.ice_tie_breaker));
    if (ret < 0) {
...
static int generate_sdp_offer(AVFormatContext *s)
{
    ...
    snprintf(whip->rtc.ice_ufrag_local, sizeof(whip->rtc.ice_ufrag_local), "%08x",
        av_lfg_get(&whip->rtc.rnd));
    snprintf(whip->rtc.ice_pwd_local, sizeof(whip->rtc.ice_pwd_local), "%08x%08x%08x%08x",
        av_lfg_get(&whip->rtc.rnd), av_lfg_get(&whip->rtc.rnd), av_lfg_get(&whip->rtc.rnd),
        av_lfg_get(&whip->rtc.rnd));

    whip->audio_ssrc = av_lfg_get(&whip->rtc.rnd);
    ...
    if (whip->audio_par) {
        ...
        av_bprintf(&bp, ""
            ...
            whip->rtc.ice_ufrag_local,
            whip->rtc.ice_pwd_local,
            whip->rtc.dtls_fingerprint,
