...
static int setup_srtp(AVFormatContext *s)
{
    ...
    ret = ff_srtp_set_crypto(&rtc->srtp_send, suite, buf);
    if (ret < 0) {
        av_log(s, AV_LOG_ERROR, "Failed to set crypto for send\n");
        goto end;
    }
