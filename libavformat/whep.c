...
typedef struct WHEPContext {
    ...
    /* The key material exported from the completed DTLS handshake. */
    uint8_t dtls_srtp_materials[(RTC_DTLS_SRTP_KEY_LEN + RTC_DTLS_SRTP_SALT_LEN) * 2];
    /* The SRTP receive context, to decrypt incoming packets. WHEP does not
     * send media yet, so no send-side SRTP contexts exist here. */
...
/**
 * Establish the SRTP receive context using the keying material exported
 * from DTLS. WHEP does not send media yet, so only the receive context is
 * set up here, unlike whip.c which also sets up send contexts.
 *
...
static int setup_srtp(AVFormatContext *s)
{
    ...
    char *client_key, *server_key, *client_salt, *server_salt;

    ret = ff_dtls_export_materials(whep->dtls_uc, whep->dtls_srtp_materials, sizeof(whep->dtls_srtp_materials));
    if (ret < 0)
        goto end;
     ...
     */
    client_key = (char *)whep->dtls_srtp_materials;
    server_key = client_key + RTC_DTLS_SRTP_KEY_LEN;
    ...
    if (!av_base64_encode(buf, sizeof(buf), recv_key, sizeof(recv_key))) {
        av_log(whep, AV_LOG_ERROR, "Failed to encode recv key\n");
        ret = AVERROR(EIO);
        goto end;
    }

    ret = ff_srtp_set_crypto(&whep->srtp_recv, suite, buf);
    if (ret < 0) {
        av_log(whep, AV_LOG_ERROR, "Failed to set crypto for recv\n");
        goto end;
    }

    if (whep->state < WHEP_STATE_SRTP_FINISHED)
        whep->state = WHEP_STATE_SRTP_FINISHED;
    av_log(whep, AV_LOG_VERBOSE, "SRTP setup done, state=%d, suite=%s, key=%zuB, elapsed=%.2fms\n",
        whep->state, suite, sizeof(recv_key), ELAPSED(whep->whep_starttime, av_gettime_relative()));
